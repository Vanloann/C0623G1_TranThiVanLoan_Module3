...
-- 5.	Hiển thị ma_khach_hang, ho_ten, b ten_loai_khach, ma_hop_dong, ten_dich_vu, ngay_lam_hop_dong, ngay_ket_thuc, tong_tien 
...
SELECT 
    ...
    COUNT(hd.ma_hop_dong) AS so_khach_hang_dat_phong
FROM
    hop_dong hd
WHERE
...
SELECT ten_dich_vu_di_kem, ho_ten
FROM khach_hang k
...
JOIN khach_hang k ON (h.ma_khach_hang = k.ma_khach_hang AND (dia_chi LIKE '%Vinh%' OR dia_chi LIKE '%Quảng Ngãi%'))
JOIN loai_khach lk ON (k.ma_loai_khach = lk.ma_loai_khach AND (ten_loai_khach = 'Diamond'));

-- 12.	Hiển thị thông tin ma_hop_dong, ho_ten (nhân viên), ho_ten (khách hàng), so_dien_thoai (khách hàng), 
-- ten_dich_vu, so_luong_dich_vu_di_kem (được tính dựa trên việc sum so_luong ở dich_vu_di_kem), 
-- tien_dat_coc của tất cả các dịch vụ đã từng được khách hàng đặt vào 3 tháng cuối năm 2020 
-- nhưng chưa từng được khách hàng đặt vào 6 tháng đầu năm 2021.
SELECT h.ma_hop_dong, n.ho_ten, k.ho_ten, k.so_dien_thoai, ten_dich_vu, SUM(IFNULL(so_luong, 0)) AS 'so_luong_dich_vu_di_kem', tien_dat_coc
FROM hop_dong h 
...
CREATE VIEW temp AS
SELECT dk.*, SUM(so_luong) AS max_amount 
FROM hop_dong_chi_tiet hc 
JOIN dich_vu_di_kem dk ON hc.ma_dich_vu_di_kem = dk.ma_dich_vu_di_kem
GROUP BY dk.ma_dich_vu_di_kem;

SELECT ten_dich_vu_di_kem, max_amount
FROM temp
WHERE max_amount IN (SELECT max(max_amount) FROM temp);

-- 14.	Hiển thị thông tin tất cả các Dịch vụ đi kèm chỉ mới được sử dụng một lần duy nhất. 
-- Thông tin hiển thị bao gồm ma_hop_dong, ten_loai_dich_vu, ten_dich_vu_di_kem, so_lan_su_dung 
-- (được tính dựa trên việc count các ma_dich_vu_di_kem).
SET sql_mode = 0;
SELECT hc.ma_hop_dong, ld.ten_loai_dich_vu, dk.ten_dich_vu_di_kem, COUNT(hc.ma_hop_dong) AS so_lan_su_dung
FROM hop_dong h
JOIN dich_vu d ON h.ma_dich_vu = d.ma_dich_vu
...
CREATE VIEW xoa AS 
SELECT n.ma_nhan_vien, n.ho_ten, COUNT(h.ma_hop_dong)
FROM nhan_vien n
...
HAVING SUM(d.chi_phi_thue + dk.gia * hc.so_luong) > 10000000;
SELECT * FROM cap_nhat_khach_hang;
...
WHERE
    ma_khach_hang IN (SELECT 
        ...
        FROM
            ...
            JOIN hop_dong h ON k.ma_khach_hang = h.ma_khach_hang
            WHERE
                YEAR(ngay_lam_hop_dong) = '2020') AS temp); 
